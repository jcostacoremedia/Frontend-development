...
addNodeDecoratorBySelector(`.${BLOCK}`, ($navigationRoot) => {
  ...
  $navigationEntries.mouseover(function () {
    ...
    $navigationEntries.not(this).each(function () {
      ...
      $this.find(`ul.${ITEM_ELEMENT_MENU}`).addClass(ITEM_ELEMENT_MENU_VISIBILITY_MODIFIER);
    });

    $currentNavigationEntry.removeClass(ITEM_MODIFIER_DEPTH_1_BORDER_MODIFIER);
    $currentNavigationEntry.find(`ul.${ITEM_ELEMENT_MENU}`).removeClass(ITEM_ELEMENT_MENU_VISIBILITY_MODIFIER);
...
addNodeDecoratorBySelector(".cm-header__mobile-navigation-button.cm-hamburger-icon", ($hamburgerIcon) => {
  const $body = $("body");
