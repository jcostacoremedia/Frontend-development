import $ from "jquery";
import { addBEMModifier, findBEMElement, removeBEMModifier } from "@coremedia/brick-utils";
...
export function getDownloadCollectionCount() {
  ...
  for (const assetId in downloadCollection) {
    if (Object.prototype.hasOwnProperty.call(downloadCollection, assetId)) {
...
export function updateRenditionLinkTextState($control, config) {
  if (isInDownloadCollection(config.assetId, config.rendition)) {
    removeBEMModifier($control, BLOCK_NAME_DLC_RENDITION_CONTROL, MODIFIER_NAME_DLC_RENDITION_CONTROL_ADDABLE);
    addBEMModifier($control, BLOCK_NAME_DLC_RENDITION_CONTROL, MODIFIER_NAME_DLC_RENDITION_CONTROL_REMOVABLE);
  } else {
    removeBEMModifier($control, BLOCK_NAME_DLC_RENDITION_CONTROL, MODIFIER_NAME_DLC_RENDITION_CONTROL_REMOVABLE);
    addBEMModifier($control, BLOCK_NAME_DLC_RENDITION_CONTROL, MODIFIER_NAME_DLC_RENDITION_CONTROL_ADDABLE);
  }
}
...
export function updateDownloadCollectionButtonState($button, $counter) {
  ...
  $button = $button || findBEMElement($collection, BLOCK_NAME_DLC, ELEMENT_NAME_DLC_BUTTON);
  $counter = $counter || findBEMElement($collection, BLOCK_NAME_DLC, ELEMENT_NAME_DLC_COUNTER);
...
export function addOrRemoveRenditionFromDownloadCollection(assetId, renditionName) {
  ...
  if (indexOfRendition === -1) {
    if (getDownloadCollectionCount() < DOWNLOAD_COLLECTION_MAX_QUANTITY) {
      ...
      logger.error("Maximum number of items in Asset Download Collection reached", DOWNLOAD_COLLECTION_MAX_QUANTITY);
    }
  } else {
