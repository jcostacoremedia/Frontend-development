...
import { addNodeDecoratorByData, addNodeDecoratorBySelector } from "@coremedia/brick-node-decoration-service";
...
function loadSearchResultPageClickHandler($target) {
  ...
  $target.on("click touch", "[data-cm-search-link]", function (event) {
    ...
    loadSearchResultPage(link, searchResultPageContainerId, !$("body").data("cm-search-disable-browser-history"));
  });
  /* on dropdown */
...
addNodeDecoratorBySelector("#" + searchResultPageContainerId, loadSearchResultPageClickHandler, cleanup);
...
function toggleFilterMobilePopupClickHandler($target) {
  ...
  $target.on("click touch", function () {
    $("[data-cm-search-filter-popup]").toggleClass("cm-search__filter-popup--active");
    $("body").toggleClass("cm-body--filter-popup-active");
  });
}
addNodeDecoratorBySelector("[data-cm-search-filter-popup-toggle]", toggleFilterMobilePopupClickHandler);
...
addNodeDecoratorBySelector("[data-cm-search-filter]", toggleSearchFilterClickHandler);
  ...
  window.addEventListener(
    ...
    function () {
      if (history.state && history.state.id === "search") {
        loadSearchResultPage(history.state.link, searchResultPageContainerId, false);
      }
    },
    false,
  );
