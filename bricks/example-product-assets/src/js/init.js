...
import { EVENT_SRC_CHANGED, getCurrentResponsiveImageFormat } from "@coremedia/brick-media";
...
function getChangeImagesFn($productAssets, shoppingActionsJS) {
  return function (catEntryId, productId) {
    ...
    const selectedAttributes = shoppingActionsJS.selectedAttributesList[entitledItemId];
    let attributes = "";
    for (let attribute in selectedAttributes) {
      if (Object.prototype.hasOwnProperty.call(selectedAttributes, attribute)) {
...
addNodeDecoratorByData(
  ...
  function ($target, { zoom }, instance) {
    ...
    $slideshow.on("afterChange", function (event, slick, currentSlide) {
      ...
      $carousel.find(`.slick-slide[data-slick-index="${currentSlide}"]`).addClass("slick-slide--active");
    });
        ...
        if ($responsiveImage && $responsiveImage.length > 0) {
          ...
          if (responsiveImageFormat) {
            const biggestWidth = Math.max(
              ...Object.keys(responsiveImageFormat.linksForWidth).map((key) => parseInt(key)),
            );
  ...
  function ($target, config, instance) {
