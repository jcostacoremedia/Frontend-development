...
function getRect(element) {
  ...
  return new Rect(
    boundingRect.left + (document.documentElement.scrollLeft || document.body.scrollLeft),
    boundingRect.top + (document.documentElement.scrollTop || document.body.scrollTop),
    boundingRect.width,
    boundingRect.height,
  );
}
...
class Zoom {
  ...
  _onLoad() {
    ...
    this.zoomWindow.style.backgroundImage = `url(${encodeURI(this.imagePreload.src)})`;
  }
  ...
  _updatePosition(cursorPosition) {
    if (this.imageLink && cursorPosition) {
      ...
      let zoomWindowWidth = containerRect.width + containerRect.left - imageRect.right - offsetX;
      ...
      let lensRect = Rect.fromCenter(
        ...
        Math.min(
          imageRect.height,
          Math.ceil(((imageRect.width * this.zoomBase) / zoomWindowRect.width) * zoomWindowRect.height),
        ),
      );
      ...
      const percentageX =
        backgroundPositionDimensions.width === 0
          ? "50%"
          : ((lensRect.center.left - imageRect.left - lensRect.width / 2) / backgroundPositionDimensions.width) * 100 +
            "%";
      const percentageY =
        backgroundPositionDimensions.height === 0
          ? "50%"
          : ((lensRect.center.top - imageRect.top - lensRect.height / 2) / backgroundPositionDimensions.height) * 100 +
            "%";
