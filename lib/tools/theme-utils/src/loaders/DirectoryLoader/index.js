...
module.exports = function () {
  ...
  if (!subtask) {
    ...
    const dependencies = getDependencies(pkgPath, getIsSmartImportModuleFor(null));
    dependenciesToLoadByPackageName = getDependentsFirstLoadOrder(dependencies, packageName);
  }
  ...
  if (resolvedDirectory && fs.existsSync(resolvedDirectory)) {
    ...
    foundSubdirectories.forEach((foundSubdirectory) =>
      this.addContextDependency(path.join(resolvedDirectory, path.basename(foundSubdirectory))),
    );
    ...
    const foundModules = globResult
      ...
      .map((file) => loaderUtils.urlToRequest(path.relative(process.cwd(), file), this.context));
  ...
  modulesToLoad.push(
    ...(dependenciesToLoadByPackageName[packageName] || []).map(
      (dependency) =>
        `${__filename}?directory=${encodeURIComponent(directory)}&pattern=${encodeURIComponent(
          pattern,
        )}&pkgPath=${encodeURIComponent(dependency.getPkgPath())}!`,
    ),
  );
