...
const { Compilation, PrefetchPlugin } = require("webpack");
...
class JoinWebpackPlugin {
  ...
  buildGroup(group) {
    ...
    let result = null;
    files.forEach((file) => (result = this.options.join(result, group.sources[file], file)));
  ...
  apply(compiler) {
    ...
    compiler.hooks.thisCompilation.tap(NAME, (compilation) => {
      compilation.hooks.optimizeTree.tapAsync(NAME, (chunks, modules, callback) => {
        this.state = "building";
        ...
        async.forEach(
          // chunks are Sets since webpack5, convert to array
          [...chunks],
          (chunk, callback) => {
            async.forEach(
      ...
      compilation.hooks.processAssets.tap(
        ...
        (assets) => {
          Object.keys(this.groups).forEach((groupName) => {
            const group = this.group(groupName);
            assets[group.filename] = new RawSource(group.result);
          });
        },
      );
    });
