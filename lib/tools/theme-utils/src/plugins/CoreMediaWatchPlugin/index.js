...
const { clearConsole, printInfo, openBrowser, printErrorStack } = require("./utils");
...
class CoreMediaWatchPlugin {
  constructor(options) {
    ...
    if (typeof this.options.monitorConfig !== "object" || typeof this.options.monitorConfig.target !== "string") {
      throw new Error(
  ...
  apply(compiler) {
    const logLevel = this.options.logLevel || cmLogger.getLevelFromWebpackStats(compiler.options.stats);
      ...
      if (this._isWatching) {
        ...
        if (errors.length > 0) {
          ...
          if (this._isRemoteWorkflow()) {
            ...
            this._log.finalError("Processing file changes aborted due to compilation errors.");
          } else {
            // local workflow: file changes have been processed in target directory, but may contain errors.
            this._log.finalError("The preview may not work properly due to compilation errors.");
          }
          return;
        }

        const emittedFiles = Object.keys(assets).map((asset) => {
          return path.join(stats.compilation.compiler.outputPath, asset);
        });
        ...
        if (init) {
          ...
          if (this._isRemoteWorkflow()) {
            try {
              await uploadTheme(this.options.themeConfig);
              clearConsole();
              printInfo();
              openBrowser();
            } catch (e) {
        ...
        const changedFiles = emittedFiles.filter((emittedFile) => this._hasChanged(emittedFile));
        ...
        const filesToUpdate = [
          ...
          ...new Set(
            ...
            changedFiles
              .map((changedFile) => this._mapTemplatesToArchive(changedFile, changedFile))
        ...
        if (filesToUpdate.length > 0) {
          const changedFilesForOutput = filesToUpdate.map(this._relativeToThemeFolder, this);
          if (this._isRemoteWorkflow()) {
            this._log.info("Preparing: ", changedFilesForOutput);
            try {
              const count = await uploadFiles(this.options.themeConfig, filesToUpdate, logLevel);
            ...
            this._log.finalInfo("Added/Changed files processed: ", changedFilesForOutput);
            livereload.trigger(filesToUpdate);
          }
        } else {
          if (this._isRemoteWorkflow()) {
