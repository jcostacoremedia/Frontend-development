...
const printInfo = () => {
  ...
  log.info();
  log.info(chalk.bold("Subsequent changes will automatically be transferred to the remote CAE"));
  log.info(chalk.bold("and be available in preview using the developer mode."));
  log.info("To instantly reload your changes in the browser, you may need to accept");
  log.info(`the certificate for the local LiveReload server (https://${livereloadURL}) first.`);
  log.info();
