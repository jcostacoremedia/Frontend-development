...
class ZipperWebpackPlugin {
  constructor(patterns, options) {
    ...
    if (
      !(Object.values(compilerEvents).includes(this.options.compilerEvent) && typeof this.options.filepath === "string")
    ) {
      throw new Error(
        `[${
          this._pkgName
  ...
  _handleEvent(compilation, cb) {
    ...
    zipper(this.patterns, {
      ...
      .catch((error) => {
        compilation.errors.push(new Error(`[${this._pkgName}] ${error.message}`));
      });
  }

  apply(compiler) {
    this._logLevel = this.options.logLevel || cmLogger.getLevelFromWebpackStats(compiler.options.stats);
