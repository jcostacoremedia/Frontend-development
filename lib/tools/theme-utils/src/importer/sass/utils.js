...
function resolveScssModuleImport(url, prev) {
  ...
  if (prefixPattern.test(url)) {
    ...
    if (modulePattern.test(url)) {
      url = url.replace(modulePattern, (moduleName) => getInstallationPath(moduleName, prev));
    } else {
      throw new Error(`Could not resolve url: "${originalUrl}" which was marked for exclude.`);
    }
  }
  return url;
}
