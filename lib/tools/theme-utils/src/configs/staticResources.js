...
const {
  ...
  packages,
  workspace: { getIsSmartImportModuleFor, isThemeModule, getThemeConfig, parseSettings, mergeSettings },
} = require("@coremedia/tool-utils");
const CopyWebpackPlugin = require("copy-webpack-plugin");
const ViewRepositoryPlugin = require("../plugins/ViewRepositoryPlugin");
...
function configureJoinWebpackPluginForResourceBundles(
  ...
  const searchPatterns = nodeModules
    ...
    .map((nodeModulePkgPath) => path.resolve(nodeModulePkgPath, relativeResourceBundleSrc, PROPERTIES_GLOB));
  ...
  return new JoinWebpackPlugin({
    ...
    join: function (common, addition, filename) {
      ...
      addition = `# Resource Bundle from: ${pkg.name}\n# File: ${relativeFilePath}\n\n` + addition;
    ...
    save: function (common) {
      ...
      const orderedContent = nodeModules.map((nodeModule) => common[nodeModule.getName()] || "");
...
function getPatternForThemeDescriptor() {
  ...
  return fs.existsSync(descriptorSourcePath)
    ? {
        ...
        to: path.relative(themeConfig.themeTargetPath, path.dirname(themeConfig.descriptorTargetPath)),
        force: true,
        cacheTransform: true,
      }
    : null;
...
module.exports = () => (config) => {
  const nonThemeDependencies = getFlattenedDependencies(themeConfig.pkgPath, getIsSmartImportModuleFor(null)).filter(
    (nodeModule) => !isThemeModule(nodeModule),
  );

  const themeDependencies = getFlattenedDependencies(themeConfig.pkgPath, isThemeModule);

  const themePaths = themeDependencies.map((themeDependency) => path.dirname(themeDependency.getPkgPath()));
  ...
  const joinWebpackPlugin = configureJoinWebpackPluginForResourceBundles(
    "Bricks",
    "src/l10n",
    path.relative(themeConfig.themeTargetPath, themeConfig.resourceBundleTargetPath),
    nonThemeDependencies,
  );

  const joinSettingsWebpackPlugin = (() => {
    const relativeSettingsSrc = "src/settings";
    const relativeSettingsTarget = path.relative(themeConfig.themeTargetPath, themeConfig.settingsTargetPath);
    const nodeModules = nonThemeDependencies.concat(themeDependencies);
    const searchPatterns = nodeModules
      .map((nodeModule) => path.dirname(nodeModule.getPkgPath()))
      // add the theme that is being build
      .concat([themeConfig.path])
      .map((nodeModulePkgPath) => path.resolve(nodeModulePkgPath, relativeSettingsSrc, SETTINGS_GLOB));
    ...
    return new JoinWebpackPlugin({
      ...
      join: (settingsByPackageName, newSettings, filename) => {
        ...
        settingsByPackageName[packageName] = parseSettings(filename, newSettings);
        return settingsByPackageName;
      },
      save: (settingsByPackageName) => {
        // join the map by iterating of the flattened dependency order
        const orderedSettings = nodeModules.map((nodeModule) => settingsByPackageName[nodeModule.getName()] || "");
        orderedSettings.push(settingsByPackageName[themeConfig.packageName] || "");
  ...
  const viewRepositoryPlugin = new ViewRepositoryPlugin({
    ...
    outputPath: path.relative(config.output.path, path.join(themeConfig.templatesTargetPath)),
  ...
  return deepMerge(config, {
    ...
    module: {
      rules: [
        ...
        },
        {
          ...
          use: [viewRepositoryPlugin.getLoaderConfig()],
        },
        // bundles
        {
          //  test: /\.swf$/,
    ...
    plugins: [
      ...
      new ZipperWebpackPlugin(
        [
          {
            source: path.relative(themeConfig.resourcesTargetPath, themeConfig.themeTemplatesTargetPath),
            prefix: path.normalize("META-INF/resources/"),
            context: path.relative(themeConfig.themeTargetPath, themeConfig.resourcesTargetPath),
          },
          ...
          // to properly handle importing from a theme template to a brick freemarker lib
          {
            ...
            context: path.relative(themeConfig.themeTargetPath, themeConfig.templatesTargetPath),
          },
        ],
        {
          filepath: path.relative(themeConfig.themeTargetPath, themeConfig.themeTemplatesJarTargetPath),
