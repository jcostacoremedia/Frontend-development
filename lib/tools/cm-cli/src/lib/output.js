...
function getThemeChoice(packageName, value = packageName) {
  const displayName = packageName.replace(`${COREMEDIA_SCOPE}/`, "").replace(`${EXAMPLE_SCOPE}/`, "(Example) ");
  return {
    name: displayName,
    short: displayName,
    value: value,
...
function sortChoices(choices) {
  return choices.sort(({ name = "" }, { name: otherName = "" }) => name.localeCompare(otherName));
}
