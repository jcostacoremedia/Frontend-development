...
const { convertModuleName, isModuleNameInUse, createTheme } = require("@coremedia/module-creator");
const args = require("../lib/args");
const { PKG_NAME } = require("../lib/constants");
const { getBrickChoice, getThemeChoice, sortChoices } = require("../lib/output");
...
const handler = ({ name, verbose }) => {
  ...
  function doIt() {
    ...
    try {
      ...
      log.error(`An error occured while trying to create theme "${themeName}": ${e.message}`);
    }
  }
  ...
  const askForBricks = () => {
    inquirer
      ...
      .then(({ chosenBricks, commentOutBricks }) => {
        ...
        dependenciesToActivate = Object.assign(dependenciesToActivate, bricksList);
        if (commentOutBricks) {
  ...
  const setPath = (newName) => {
    ...
    if (isModuleNameInUse(themePath)) {
      log.error(`The theme "${newName}" already exists. Please choose another name.`);
      getName();
    } else {
      askToDerive();
    }
