...
const { convertModuleName, isModuleNameInUse, createBrick } = require("@coremedia/module-creator");
...
const handler = (argv) => {
  ...
  function doIt() {
    ...
    try {
      ...
      log.error(`An error occured while trying to create brick "${brickName}": ${e.message}`);
    }
  }
  ...
  const setPath = (brickName) => {
    ...
    if (isModuleNameInUse(brickPath)) {
      log.error(`The brick "${brickName}" already exists. Please choose another name.`);
      getName();
    } else {
      doIt();
    }
