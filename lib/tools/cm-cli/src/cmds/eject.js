...
const {
  ...
  workspace: { getWorkspaceConfig, getAvailableBricks, getInstallationPath, isExampleBrickModule, isExampleModuleName },
...
const handler = ({ verbose }) => {
  ...
  const installationPathByBrickName = Object.keys(getAvailableBricks()).reduce((aggregator, availableBrick) => {
    try {
      aggregator[availableBrick] = getInstallationPath(availableBrick, wsConfig.path);
    } catch (e) {
      log.warn(`Ignoring brick "${availableBrick}" as it was not installed. Consider using "pnpm install".`);
    }
    return aggregator;
  }, {});
  ...
  function getDependenciesFor(brickName) {
    return getFlattenedDependencies(path.join(installationPathByBrickName[brickName], "package.json"), (nodeModule) =>
      getAvailableBrickNames().includes(nodeModule.getName()),
    )
  ...
  Object.keys(flattenedExampleBrickDependenciesByBrickName).forEach((name) => {
    ...
    dependencies.forEach((dependency) => {
      dependentsByExampleBrickName[dependency] = dependentsByExampleBrickName[dependency] || [];
  ...
  function doEject() {
    const bricksToEject = Object.keys(newNameByOldName).filter((oldName) => {
      ...
      return newName !== undefined && !getAvailableBrickNames().includes(newName);
    });
  ...
  function askForDependencyHandling(unhandledDependency) {
    ...
    const dependents = (dependentsByExampleBrickName[unhandledDependency] || []).filter((dependent) =>
      trackedDependencies.includes(dependent),
    );
    inquirer
      .prompt({
        type: "list",
  ...
  function checkInformation() {
    const trackedDependencies = Object.keys(newNameByOldName);
    const unhandledEjections = trackedDependencies.filter((key) => newNameByOldName[key] === undefined);
    ...
    const unhandledDependencies = Object.keys(flattenedExampleBrickDependenciesByBrickName)
      .filter((dependencyName) => trackedDependencies.includes(dependencyName))
      .reduce((aggregator, next) => {
        return [...aggregator, ...flattenedExampleBrickDependenciesByBrickName[next]];
      }, [])
      .filter((dependencyName) => !trackedDependencies.includes(dependencyName));
