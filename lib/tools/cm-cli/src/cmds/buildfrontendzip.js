...
const handler = () => {
  ...
  zipper(
    [
      {
        ...
        source: "*.xml",
        prefix: path.relative(wsConfig.resourcesTargetPath, wsConfig.descriptorsTargetPath),
      },
      {
    ...
    .then((count) => {
      log.info(`Success, zipped ${count} files to location "${themesZipTargetPath}".`);
    })
    .catch((e) => {
      throw new Error(`[${PKG_NAME}] An error occurred while preparing zip file for themes: ${e.message}`);
    });
};
