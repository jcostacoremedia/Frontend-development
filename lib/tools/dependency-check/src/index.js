...
function hasPkgDependency(sourceFile, requiredFile) {
  if (
    // ignoring some packages here...
    ["@babel/runtime", "expose-loader", "exports-loader", "imports-loader"].some((excludedPackages) =>
      requiredFile.includes(path.normalize("node_modules/" + excludedPackages)),
    )
  ) {
    return true;
  }
  ...
  if (sourcePkgName === requiredPkgName) {
    return true;
  }
...
class DependencyCheckWebpackPlugin {
  ...
  apply(compiler) {
    ...
    compiler.hooks.done.tap(NAME, function (stats) {
      ...
      for (let sourceModule of sourceModules) {
        ...
        // do not check dependencies of scss files, the loader does not track the source files properly
        if (sourceFile && !sourceFile.endsWith(".scss")) {
          for (let dependency of sourceModule.dependencies) {
            const requiredModule = stats.compilation.moduleGraph.getModule(dependency);
            const requiredFile = requiredModule && requiredModule.resource;
            if (requiredFile && !alreadyTrackedResources.includes(requiredFile)) {
        ...
        if (plugin.isIncluded(unmanagedDependency.sourceFile)) {
          stats.compilation.errors.push(
            new MissingDependencyError(
              getMissingDependencyErrorMessage(unmanagedDependency.sourceFile, unmanagedDependency.requiredFile),
            ),
          );
        }
      }
      ...
      if (plugin.isIncluded(prev)) {
        ...
        if (prefixPattern.test(url)) {
          ...
          if (modulePattern.test(url)) {
            ...
            const filePathByPackageName = packages.getFilePathByPackageName(moduleName, path.dirname(prev));
            plugin.dependencies.push({
              sourceFile: prev,
  ...
  isIncluded(path) {
    ...
    return (
      (!this.includes || this.includes.length === 0 || this.includes.some(patternOrStringMatches)) &&
