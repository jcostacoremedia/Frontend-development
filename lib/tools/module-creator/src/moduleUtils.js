...
const createThemeFolderStructure = (wsConfig, themePath, log) => {
  const directories = [
    ...
    path.join(themePath, "src", "templates", "com.coremedia.blueprint.common.contentbeans"),
  ];
  directories.forEach((dir) => {
    log.debug(`Create directory "themes/${path.relative(wsConfig.themesPath, dir)}"`);
...
const createBrickFolderStructure = (wsConfig, brickPath, log) => {
  const directories = [
    ...
    path.join(brickPath, "src", "templates", "com.coremedia.blueprint.common.contentbeans"),
  ];
  directories.forEach((dir) => {
    log.debug(`Create directory "bricks/${path.relative(wsConfig.bricksPath, dir)}"`);
    fs.mkdirSync(dir, { recursive: true });
  });
};
...
const createThemeFiles = (
  ...
  const files = [
    ...
    },
    {
      ...
      data: themeData.initThemeConfigJson(themeName, Object.keys(dependenciesToActivate).length > 0),
    },
    {
...
const createTheme = (
  ...
  createThemeFiles(themePath, themeName, dependenciesToActivate, dependenciesToCommentOut, themeToDeriveFrom, log);
};
    ...
    if (hit !== "package.json") {
      ...
      packageJson.dependencies = Object.keys(packageJson.dependencies).reduce(
        (dependencies, nextDependency) => ({
          ...dependencies,
          [newNameByOldName[nextDependency] || nextDependency]: packageJson.dependencies[nextDependency],
        }),
        {},
      );
      fs.writeFileSync(targetFile, JSON.stringify(packageJson, null, 2), "utf8");
    }
  });
};
