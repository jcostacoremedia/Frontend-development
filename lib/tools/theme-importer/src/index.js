...
const login = (studioUrl, previewUrl, proxyUrl, username, password) => {
  return new Promise((resolve, reject) => {
    ...
    api
      ...
      .then((user) => {
        const previewUrlDevMode = utils.getPreviewUrlDevMode(previewUrl, user.id);
        resolve({
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (!fs.existsSync(themeConfig.themeArchiveTargetPath)) {
        reject(new Error(`[${PKG_NAME}] ${themeConfig.themeArchiveTargetPath} doesn´t exist.`));
      } else {
        log.debug(`Using ${studioUrl}`);
        log.info("Uploading theme to remote server.");

        api
          .upload(studioUrl, proxyUrl, apiKey, themeConfig.themeArchiveTargetPath, "true")
          .then(() => {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (!fs.existsSync(themeConfig.themeArchiveTargetPath)) {
        ...
        api
          .deploy(studioUrl, proxyUrl, apiKey, themeConfig.themeArchiveTargetPath)
          .then(() => {
            resolve(themeConfig.themeArchiveTargetPath);
          })
          .catch((e) => {
    ...
    try {
      ...
      zipper(patterns, {
        ...
        .then((count) => {
          ...
          _log.debug(`Uploading file ${path.basename(themeConfig.themeUpdateArchiveTargetPath)}`);

          api
            .upload(studioUrl, proxyUrl, apiKey, themeConfig.themeUpdateArchiveTargetPath)
            .then(() => {
              resolve(count);
            })
            .catch((e) => {
        ...
        .catch((e) => {
          reject(new Error(`[${PKG_NAME}] An error occured while preparing for upload: ${e.message}`));
        });
    } catch (e) {
      reject(e);
    }
...
const deleteFile = (themeConfig, file) => {
  return new Promise((resolve) => {
    try {
      ...
      const { studioUrl, proxyUrl } = getEnv();
      const apiKey = getApiKey();
      const FILE_PATH = file.replace(themeConfig.themeTargetPath + path.sep, "");
