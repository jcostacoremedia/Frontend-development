  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (!(options && typeof options.filepath === "string")) {
        throw new Error(`[${PKG_NAME}] options must be an object which at least contains property "filepath"`);
      }
      if (options.context && typeof options.context !== "string") {
        throw new ZipFileError(`[${PKG_NAME}] options.context must be a string`);
      }
      ...
      patterns.forEach((pattern) => {
        ...
        if (isGlobPattern(pattern.source)) {
          ...
          if (stats.isDirectory()) {
            const destPath = path.normalize((pattern.prefix ? pattern.prefix + path.sep : "") + pattern.source);
            log.debug(`Adding directory "${sourcePath}" to archive as "${destPath}"`);
            archive.directory(sourcePath, destPath);
          } else if (stats.isFile()) {
            const name = path.normalize((pattern.prefix ? pattern.prefix + path.sep : "") + pattern.source);
