...
const JSON_SCHEMA_THEME_CONFIG = "https://www.coremedia.com/json-schemas/theme.config.schema.json";
const JSON_SCHEMA_SETTINGS = "https://www.coremedia.com/json-schemas/settings.schema.json";
...
const getWSPackageJson = (from) => {
  ...
  const check = () => {
    ...
    return getCoreMediaEntryFromPackageJson(packageJsonPath).type === "workspace" ? packageJsonPath : next();
  };
...
const getWorkspaceConfig = () => {
  if (!wsConfig) {
    ...
    if (!packageJsonPath) {
      ...
      const coremediaEntry = getCoreMediaEntryFromPackageJson(fallbackPackageJsonPath);
...
function parseSettings(filename, settings = undefined) {
  ...
  if (typeOfSettingsJson !== "object") {
    log.warn(`The JSON must be of type "object" but found "${typeOfSettingsJson}"`);
    settingsJson = {};
  }
...
function mergeSettings(settings) {
  return settings
    ...
    .reduce((mergedSettings, additionalSettings) => {
      const newMergedSettings = mergeWith(mergeWith({}, mergedSettings), additionalSettings);
      try {
...
function assertConfiguredPathExists(path) {
  if (!fs.existsSync(path)) {
    log.error(`Could not find file: '${path}'. Please check your theme configuration.`);
    process.exit(-1);
  }
}
...
function postProcessThemeConfig(themeConfig, defaultRootPath) {
  ...
  if (themeConfig.thumbnail !== null) {
    assertConfiguredPathExists(path.join(themeConfig.path, themeConfig.thumbnail));
  }
  ...
  themeConfig.styles
    ...
    .forEach((src) => assertConfiguredPathExists(path.join(themeConfig.path, src)));
  ...
  themeConfig.styles
    ...
    .forEach((webpack) => {
      const firstSrc = webpack.src instanceof Array ? webpack.src[0] : webpack.src;
      webpack.entryPointName = firstSrc ? path.basename(firstSrc).replace(/\.(scss|css|js)$/, "") : themeConfig.name;
    });
}
...
function getThemeConfig() {
  if (!themeConfig) {
    ...
    if (!plainThemeConfig) {
      throw new Error(`No theme config found in '${themeJson5Path}, '${themeJsonPath}' or '${packageJsonPath}'`);
    }
      ...
      const concatinatedErrors = themeConfigValidator.errors
        .map((error) => {
          ...
          if (error.keyword === "enum" && error.params.allowedValues) {
            error.message += ": " + error.params.allowedValues.map((allowedValue) => `"${allowedValue}"`).join(", ");
          }
    ...
    const descriptorTargetPath = path.join(wsConfig.descriptorsTargetPath, `${plainThemeConfig.name}-theme.xml`);

    const themeTargetPath = path.join(wsConfig.themesTargetPath, plainThemeConfig.name);

    const templatesTargetPath = path.join(resourcesTargetPath, "WEB-INF/templates");

    const themeTemplatesTargetPath = path.join(templatesTargetPath, plainThemeConfig.name);

    const themeTemplatesJarTargetPath = path.join(themeTargetPath, `templates/${plainThemeConfig.name}-templates.jar`);

    const resourceBundleTargetPath = path.join(themeTargetPath, "l10n");
    const settingsTargetPath = path.join(themeTargetPath, "settings");

    const themeContentTargetPath = path.resolve(targetPath, "content/Themes", plainThemeConfig.name);

    const themeArchiveTargetPath = path.resolve(targetPath, `themes/${plainThemeConfig.name}-theme.zip`);

    const themeUpdateArchiveTargetPath = path.resolve(targetPath, `themes/${plainThemeConfig.name}-update.zip`);
...
function getCoreMediaEntry(nodeModule) {
  ...
  if (!(moduleName in cachedPackageJsonsByName)) {
    cachedPackageJsonsByName[moduleName] = getCoreMediaEntryFromPackageJson(nodeModule.getPkgPath());
  }
...
function getShims(nodeModule) {
  ...
  return Object.keys(shimConfigByTarget).map((target) =>
    parseShim(target, shimConfigByTarget[target], path.dirname(nodeModule.getPkgPath())),
  );
}
...
function getIsSmartImportModuleFor(variant = DEFAULT_VARIANT) {
  return (nodeModule) =>
    (isBrickModule(nodeModule) || isLibraryModule(nodeModule) || isThemeModule(nodeModule)) &&
...
function getInstallationPath(moduleName, relativeFrom) {
  return packages.getFilePathByPackageName(
    moduleName,
    relativeFrom ? path.dirname(closestPackage.sync(relativeFrom)) : process.cwd(),
  );
}
...
const createCertFile = (configPath, certFile) => {
  try {
    ...
    const pems = selfsigned.generate(attributes, {
      ...
      days: 30,
      keySize: 2048,
      extensions: [{ name: "subjectAltName", altNames: [{ type: 6, value: "localhost" }] }],
    });
    ...
    log.error(`An error occured while trying to store the certificate for the LiveReload server: ${e.message}`);
  }
};
