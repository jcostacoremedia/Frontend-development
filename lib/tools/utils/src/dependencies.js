 ...
 */
function resolveDependencies(pkgPath, accept = ACCEPT_ALL, parentDependency = null, resolvedDependenciesByName = {}) {
  ...
  if (pkg.dependencies instanceof Object) {
    Object.keys(pkg.dependencies).forEach((dependencyName) => {
      if (dependencyName in resolvedDependenciesByName) {
        ...
        const dependencyPkgPath = closestPackage.sync(getInstallationPath(dependencyName, pkgPath));

        const resolvedDependency = resolveDependencies(dependencyPkgPath, accept, me, resolvedDependenciesByName);
...
function flattenDependencies(dependencies, collected = [], addedDependenciesByName = {}) {
  if (dependencies instanceof Array) {
    for (let dependency of dependencies) {
      ...
      if (!(dependencyName in addedDependenciesByName)) {
        ...
        flattenDependencies(dependency.getDependencies(), collected, addedDependenciesByName);
 ...
 */
function calculateLongestPath(edges, currentVertex, currentWeight = 1, longestPathByVertex = {}) {
  if (!(currentVertex in longestPathByVertex) || longestPathByVertex[currentVertex] < currentWeight) {
    longestPathByVertex[currentVertex] = currentWeight;
    (edges[currentVertex] || []).forEach((vertex) => {
      longestPathByVertex = calculateLongestPath(edges, vertex, currentWeight + 1, longestPathByVertex);
    });
  }
...
function createLowestWeightFirstTree(weightByVertex) {
  ...
  return Object.keys(vertexByWeight).reduce((aggregator, weight) => {
    ...
    if (nextVerticesWithSameWeight) {
      const lastVertex = verticesWithSameWeight[verticesWithSameWeight.length - 1];
      if (lastVertex) {
...
function getNodeModulesByName(nodeModules) {
  return flattenDependencies(nodeModules).reduce((aggregator, nextDependency) => {
 ...
 * @returns {Object} edges in form of a mapping of String => Array<String>
 */
function createDependencyTree(basePackageName, nodeModuleByModuleName, dependencies) {
  ...
  try {
    ...
    const loadOrder = createLowestWeightFirstTree(
      calculateLongestPath(
        createDependencyTree(basePackageName, nodeModuleByModuleName, dependencies),
        basePackageName,
      ),
    );
    ...
    return Object.keys(loadOrder).reduce((aggregator, moduleName) => {
      aggregator[moduleName] = loadOrder[moduleName].map((moduleName) => nodeModuleByModuleName[moduleName]);
      return aggregator;
    }, {});
  } catch (e) {
    if (e instanceof RangeError) {
      throw new Error(
        `Could not calculate the correct module load order, please check if there is a cycle in your dependency list:\n\n` +
          `Transitive dependency list:\n${Object.keys(nodeModuleByModuleName).join("\n")}`,
      );
    }
  }
}
