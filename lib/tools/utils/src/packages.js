...
class Packages {
  ...
  getFilePathByPackageName(packageName, resolveFromFolder) {
    ...
    if (!this._filePathByResolveFromByPackageName.has(resolveFromFolder)) {
      this._filePathByResolveFromByPackageName.set(resolveFromFolder, new Map());
    }
    const filePathByPackageName = this._filePathByResolveFromByPackageName.get(resolveFromFolder);

    if (!filePathByPackageName.has(packageName)) {
      const filePath = normalizeFilePath(resolveDirPathFromPackageName(packageName, resolveFromFolder));
    ...
    if (!this._jsonByFilePath.has(filePath)) {
      ...
      if (json && json.name) {
        ...
        if (this._filePathByResolveFromByPackageName.has(packageName)) {
          let cachedFilePath = this._filePathByResolveFromByPackageName.get(packageName);
  ...
  getJsonByPackageName(packageName, resolveFromFolder = undefined) {
    const filePath = this.getFilePathByPackageName(packageName, resolveFromFolder);
    if (!filePath) {
      return null;
    }
