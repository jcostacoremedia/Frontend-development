...
function responsiveImage(image) {
  ...
  const bestFittingFormat = formats
    ...
    .reduce(
      (currentBestFormat, nextFormat) =>
        nextFormat.difference < currentBestFormat.difference ? nextFormat : currentBestFormat,
    ...
    .reduce(
      (currentBestWidth, nextWidth) => {
        if (
          ...
          (currentBestWidth < containerWidth && nextWidth < containerWidth && nextWidth > currentBestWidth) ||
  ...
  if (bestFittingWidth === -1) {
    logger.warn(`Found no matching link in aspect ratio '${bestFittingFormat.name}'.`, image);
    return;
  }
