...
export default class {
  ...
  _requestParamsChanged(newRequestParams) {
    ...
    for (name in this._lastRequestParams) {
      if (!Object.prototype.hasOwnProperty.call(this._lastRequestParams, name)) {
        continue;
      }
    ...
    for (name in newRequestParams) {
      if (!Object.prototype.hasOwnProperty.call(newRequestParams, name)) {
        continue;
      }
      if (this._lastRequestParams[name] !== newRequestParams[name]) {
        return true;
  ...
  _changeRef(requestParams) {
    ...
    const that = this;
    updateTargetWithAjaxResponse($fragmentContainer, requestConfig, false, function (jqXHR) {
      if (jqXHR.status === 200) {
