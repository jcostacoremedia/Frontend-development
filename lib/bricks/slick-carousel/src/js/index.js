...
export default function (container, config) {
  ...
  const hasOrientationSpecificOptions =
    config.responsive && config.responsive.some((config) => [PORTRAIT, LANDSCAPE].indexOf(config.orientation) !== -1);

  if (hasOrientationSpecificOptions) {
    const responsiveConfigByOrientation = {
      [PORTRAIT]: config.responsive.filter((config) => !config.orientation || config.orientation === PORTRAIT),
      [LANDSCAPE]: config.responsive.filter((config) => !config.orientation || config.orientation === LANDSCAPE),
    };
